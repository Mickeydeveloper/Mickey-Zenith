...
exports.sendButtons = async (conn, jid, text = '', footer = '', buttons = [], quoted = null, options = {}) => {
    try {
        ...
        // Fallback: try simple buttons format
        const simpleButtons = (buttons || []).map(b => {
            if (b && b.quickReplyButton) return { buttonId: b.quickReplyButton.id, buttonText: { displayText: b.quickReplyButton.displayText }, type: 1 };
            if (b && (b.buttonId || b.id)) return { buttonId: b.buttonId || b.id, buttonText: { displayText: b.buttonText?.displayText || b.title || b.text || b.buttonId || b.id }, type: 1 };
            // generic fallback
            return { buttonId: String(b), buttonText: { displayText: String(b) }, type: 1 };
        });
        ...
        try {
            ...
            console.error('sendButtons simple buttons send failed, sending plain text fallback:', e2 && e2.message ? e2.message : e2);
